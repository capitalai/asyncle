...
#define MAKE_CONCEPT_PRED(NAME, CONCEPT) \
  template<class T> struct NAME : std::bool_constant< CONCEPT< std::decay_t<T> > > {}

#define MAKE_TRAIT_PRED(NAME, TRAIT) \
  template<class T> struct NAME : std::bool_constant< TRAIT< std::decay_t<T> >::value > {}
...
template<class T>
struct pred_arithmetic
    : std::bool_constant< pred_integral<T>::value || pred_floating_point<T>::value > {};
...
template<class T> struct pred_optional : std::false_type {};
template<class U> struct pred_optional<std::optional<U>> : std::true_type {};

template<class T> struct pred_variant : std::false_type {};
template<class... Us> struct pred_variant<std::variant<Us...>> : std::true_type {};

template<class T, class = void>
struct pred_tuple_like : std::false_type {};
template<class T>
struct pred_tuple_like<T, std::void_t<decltype(std::tuple_size<std::decay_t<T>>::value)>>
    : std::true_type {};

template<class T>
struct pred_string_view_like
    : std::bool_constant< std::is_constructible_v<std::string_view, std::decay_t<T>> > {};
...
struct rule_and {
    ...
    struct apply : std::bool_constant<
        P<std::decay_t<T>>::value && Q<std::decay_t<T>>::value> {};
};
...
struct rule_range_of {
    ...
    struct apply : std::bool_constant<
        ...
        ElemPred<std::ranges::range_value_t<std::decay_t<T>>>::value
    > {};
};
