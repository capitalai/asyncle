...
struct entry_match<P, type_map<K,V>>
    : std::bool_constant<std::same_as<std::decay_t<P>, K>> {};

template <class P, template<class> class Pred, class V>
struct entry_match<P, pred_map<Pred,V>>
    : std::bool_constant<pred_map<Pred,V>::template match<P>> {};
