...
inline expected<file_handle, file_error> open_file_impl(const char* path, const file_request& request) noexcept {
    int flags = detail::to_open_flags(request.access);
    if(request.native_flags != 0) { flags |= request.native_flags; }

    int fd = ::open(path, flags, request.permissions);
    if(fd < 0) { return expected<file_handle, file_error>(unexpect, detail::make_system_error(errno)); }

    return expected<file_handle, file_error>(file_handle(fd, flags));
}

inline expected<file_handle, file_error> create_temp_impl(const char* dir_path, const file_request& request) noexcept {
...
inline expected<file_info, file_error> stat_file_impl(const file_handle& handle) noexcept {
    struct stat st;
...
inline expected<file_info, file_error> stat_path_impl(const char* path, bool follow_symlinks) noexcept {
    struct stat st;
    int         result = follow_symlinks ? ::stat(path, &st) : ::lstat(path, &st);
    if(result < 0) { return expected<file_info, file_error>(unexpect, detail::make_system_error(errno)); }
    return expected<file_info, file_error>(detail::stat_to_info(st));
}

inline expected<uint64_t, file_error> get_file_size_impl(const file_handle& handle) noexcept {
    struct stat st;
    if(::fstat(handle.fd, &st) < 0) {
...
inline expected<io_result, file_error> read_file_impl(const file_handle& handle, const io_request& request) noexcept {
    ssize_t result;
    ...
    if(result < 0) { return expected<io_result, file_error>(unexpect, detail::make_system_error(errno)); }
...
inline expected<io_result, file_error> write_file_impl(const file_handle& handle, const io_request& request) noexcept {
    ssize_t result;
...
inline expected<io_result, file_error>
  read_vectored_impl(const file_handle& handle, const io_request* requests, size_t count) noexcept {
    if(count > IOV_MAX) { return expected<io_result, file_error>(unexpect, file_error(error_code::invalid_argument)); }
...
inline expected<uint64_t, file_error>
  seek_file_impl(const file_handle& handle, int64_t offset, seek_origin origin) noexcept {
    ...
    if(result < 0) { return expected<uint64_t, file_error>(unexpect, detail::make_system_error(errno)); }
    return expected<uint64_t, file_error>(result);
}

inline expected<uint64_t, file_error> tell_file_impl(const file_handle& handle) noexcept {
    off_t result = ::lseek(handle.fd, 0, SEEK_CUR);
    if(result < 0) { return expected<uint64_t, file_error>(unexpect, detail::make_system_error(errno)); }
    return expected<uint64_t, file_error>(result);
}

inline expected<void, file_error> sync_file_impl(const file_handle& handle, sync_flags flags) noexcept {
    int result;
    ...
    if(result < 0) { return expected<void, file_error>(unexpect, detail::make_system_error(errno)); }
...
inline expected<void, file_error>
  sync_range_impl(const file_handle& handle, uint64_t offset, uint64_t length, sync_flags flags) noexcept {
    ...
    if(flags == sync_flags::full_sync) { sync_flags |= SYNC_FILE_RANGE_WAIT_BEFORE | SYNC_FILE_RANGE_WAIT_AFTER; }
...
inline expected<void, file_error> truncate_file_impl(const file_handle& handle, uint64_t size) noexcept {
...
inline expected<void, file_error> allocate_file_impl(const file_handle& handle, uint64_t offset, uint64_t length) noexcept {
#ifdef __linux__
...
inline expected<void, file_error> lock_file_impl(const file_handle& handle, const file_lock& lock) noexcept {
    struct flock fl {};
...
inline expected<file_lock, file_error> test_lock_impl(const file_handle& handle, const file_lock& lock) noexcept {
    struct flock fl {};
...
inline expected<size_t, file_error> splice_files_impl(
  ...
  uint32_t           flags) noexcept {
    ...
    ssize_t result =
      ::splice(in.fd, in_offset ? &in_off : nullptr, out.fd, out_offset ? &out_off : nullptr, length, flags);

    if(result < 0) { return expected<size_t, file_error>(unexpect, detail::make_system_error(errno)); }
...
inline expected<size_t, file_error>
  sendfile_op_impl(const file_handle& out, const file_handle& in, uint64_t* offset, size_t count) noexcept {
#ifdef __linux__
...
inline file_caps query_file_caps_impl() noexcept {
    ...
    if(max_files > 0) { caps.max_open_files = max_files; }
