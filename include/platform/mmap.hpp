...
struct memory_error {
    error_domain domain;
    error_code code;
    uint8_t platform_errno;  // Original platform error if applicable
    uint8_t reserved;
    
    constexpr memory_error() noexcept
        : domain(error_domain::system), code(error_code::success), platform_errno(0), reserved(0) {}
        
    constexpr memory_error(error_code c) noexcept
        : domain(error_domain::system), code(c), platform_errno(0), reserved(0) {}
        
    constexpr memory_error(error_domain d, error_code c, uint8_t errno_val = 0) noexcept
        : domain(d), code(c), platform_errno(errno_val), reserved(0) {}
...
cxx23_compat::expected<void, memory_error>
prefetch_memory(const memory_region& region, std::size_t offset = 0, 
