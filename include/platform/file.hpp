...
struct file_error {
    ...
    constexpr file_error() noexcept 
        : domain(error_domain::system), platform_errno(0), code(error_code::success) {}
    
    constexpr file_error(error_code c) noexcept 
        : domain(error_domain::system), platform_errno(0), code(c) {}
    
    constexpr file_error(error_domain d, error_code c, uint8_t e = 0) noexcept 
        : domain(d), platform_errno(e), code(c) {}
};
...
struct file_request {
    ...
    constexpr file_request() noexcept 
        : access(access_mode::read_only), reserved1(0), 
          permissions(0644), native_flags(0), reserved2(0) {}
};
...
struct io_request {
    ...
    constexpr io_request() noexcept 
        : buffer(nullptr), offset(static_cast<uint64_t>(-1)), 
...
struct io_result {
    ...
    constexpr io_result() noexcept 
        : bytes_transferred(0), new_offset(0) {}
    
    constexpr io_result(size_t bytes, uint64_t offset) noexcept 
        : bytes_transferred(bytes), new_offset(offset) {}
};
...
struct file_lock {
    ...
    constexpr file_lock() noexcept 
        : type(lock_type::shared), command(lock_cmd::set), reserved1(0),
          start(0), length(0), pid(0), reserved2(0) {}
};
...
struct file_caps {
    ...
    constexpr file_caps() noexcept 
        : supports_direct_io(false), supports_async_io(false),
          supports_splice(false), supports_fallocate(false),
          supports_fadvise(false), supports_mmap(true),
          supports_lock(true), supports_extended_seek(false),
          max_file_size(0), max_open_files(0), pipe_buffer_size(0),
          reserved{} {}
};
...
expected<io_result, file_error> read_vectored(const file_handle& handle, const io_request* requests, size_t count) noexcept;
expected<io_result, file_error> write_vectored(const file_handle& handle, const io_request* requests, size_t count) noexcept;
...
expected<void, file_error> sync_range(const file_handle& handle, uint64_t offset, uint64_t length, sync_flags flags) noexcept;
...
// File advice
expected<void, file_error> advise_file(const file_handle& handle, uint64_t offset, uint64_t length, file_advice advice) noexcept;

// Zero-copy operations (Linux specific)
expected<size_t, file_error> splice_files(const file_handle& in, uint64_t* in_offset, 
                                          const file_handle& out, uint64_t* out_offset,
                                          size_t length, uint32_t flags = 0) noexcept;
expected<size_t, file_error> sendfile_op(const file_handle& out, const file_handle& in,
