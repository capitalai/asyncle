...
constexpr memory_error make_system_error(int errno_val) noexcept {
    ...
    switch (errno_val) {
        ...
        case ENOENT:
            ...
            break;
        case EBUSY:
            code = error_code::device_busy;
            break;
        case ENODEV:
            code = error_code::no_such_device;
            break;
        case EADDRINUSE:
            code = error_code::address_in_use;
            break;
        case EFAULT:
            code = error_code::bad_address;
            break;
        case ENOSYS:
...
constexpr int to_prot_flags(access_mode access) noexcept {
    int prot = PROT_NONE;
    if (static_cast<uint8_t>(access) & static_cast<uint8_t>(access_mode::read)) {
        prot |= PROT_READ;
    }
    if (static_cast<uint8_t>(access) & static_cast<uint8_t>(access_mode::write)) {
        prot |= PROT_WRITE;
    }
    if (static_cast<uint8_t>(access) & static_cast<uint8_t>(access_mode::execute)) {
        prot |= PROT_EXEC;
    }
    return prot;
}
...
constexpr int to_map_flags(const memory_request& req) noexcept {
    ...
    switch (req.sharing) {
        case sharing_mode::shared:
            flags |= MAP_SHARED;
            break;
        case sharing_mode::private_cow:
            flags |= MAP_PRIVATE;
            break;
    }
    
    // Backing type
    if (req.backing == backing_type::anonymous) {
        flags |= MAP_ANONYMOUS;
    }
    
    // Placement strategy
    switch (req.placement) {
        case placement_strategy::fixed_address:
            flags |= MAP_FIXED;
            break;
    ...
    // Large pages
    if (req.page_pref == page_preference::prefer_large || 
        req.page_pref == page_preference::require_large) {
#ifdef MAP_HUGETLB
        flags |= MAP_HUGETLB;
    ...
    if (req.populate == populate_strategy::prefault || 
        req.commit == commit_strategy::pre_commit) {
#ifdef MAP_POPULATE
        flags |= MAP_POPULATE;
#endif
    }
    ...
    if (req.enable_native) {
        flags |= static_cast<int>(req.native_flags);
    }
...
cxx23_compat::expected<void, memory_error> apply_mlock(void* addr, std::size_t length, locking_strategy strategy) noexcept {
    ...
    switch (strategy) {
        case locking_strategy::lock_resident:
            result = ::mlock(addr, length);
            break;
...
map_memory_impl(int file_descriptor, const memory_request& request) noexcept {
    ...
    if (request.length == 0) {
        return cxx23_compat::expected<memory_region, memory_error>(
            cxx23_compat::unexpect, memory_error(error_code::invalid_argument));
    }
    ...
    if (request.placement == placement_strategy::any_address) {
        hint_addr = nullptr;
    }
    
    // Perform the mapping
    void* mapped_addr = ::mmap(hint_addr, request.length, prot_flags, map_flags,
                              (request.backing == backing_type::anonymous) ? -1 : file_descriptor,
                              static_cast<off_t>(request.offset));
    
    if (mapped_addr == MAP_FAILED) {
        return cxx23_compat::expected<memory_region, memory_error>(
            cxx23_compat::unexpect, detail::make_system_error(errno));
    }
    ...
    if (request.pattern != access_pattern::normal_access) {
        ...
        switch (request.pattern) {
            case access_pattern::sequential_access:
                advice = MADV_SEQUENTIAL;
                break;
            case access_pattern::random_access:
                advice = MADV_RANDOM;
                break;
            default:
                break;
    ...
    if (request.populate == populate_strategy::hint_needed && 
...
sync_memory_impl(const memory_region& region, bool invalidate_caches) noexcept {
    if (!region.supports_sync || region.file_descriptor < 0) {
        return cxx23_compat::expected<void, memory_error>(
            cxx23_compat::unexpect, memory_error(error_code::no_such_device));
    }
    ...
    if (::msync(region.address, region.length, flags) == 0) {
        return cxx23_compat::expected<void, memory_error>();
    }
...
inline void unmap_memory_impl(const memory_region& region) noexcept {
    if (region.address && region.length > 0) {
...
inline cxx23_compat::expected<void, memory_error>
unlock_memory_impl(const memory_region& region) noexcept {
    if (::munlock(region.address, region.length) == 0) {
        return cxx23_compat::expected<void, memory_error>();
    }
