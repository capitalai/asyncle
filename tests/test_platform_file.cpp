...
int main() {
    ...
    std::cout << "Testing file creation and I/O...\n";
    {
        ...
        if(open_result) {
            ...
            if(read_open) {
                ...
                if(read_result) {
                    std::cout << "Read " << read_result.value().bytes_transferred << " bytes: '" << read_buffer << "'\n";
            ...
            std::cout << "Failed to open file for writing: " << error_to_string(open_result.error().code) << "\n";
        }
    }
    ...
    std::cout << "\nTesting temporary file creation...\n";
    {
        ...
        if(temp_result) {
            ...
            std::cout << "Failed to create temporary file: " << error_to_string(temp_result.error().code) << "\n";
        }
    }
    ...
    std::cout << "\nTesting vectored I/O...\n";
    {
        ...
        file_request req;
        req.access = static_cast<access_mode>(
          static_cast<uint8_t>(access_mode::read_write) | static_cast<uint8_t>(access_mode::create)
          | static_cast<uint8_t>(access_mode::truncate));
        req.permissions = 0644;
