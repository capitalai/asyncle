...
int main() {
    ...
    // Test anonymous mapping
    {
        ...
        if (result) {
            ...
            std::cout << "Anonymous mapping successful at: " 
                      << region.address << " (size: " << region.length << ")\n";
            ...
            if (advice_result) {
                ...
                std::cout << "Memory advice failed: " 
                          << error_to_string(advice_result.error().code) << "\n";
            }
            ...
            std::cout << "Anonymous mapping failed: " 
                      << error_to_string(result.error().code) << "\n";
        }
    }
    ...
    // Test file-backed mapping
    {
        ...
        if (result) {
            const auto& region = *result;
            std::cout << "File-backed mapping successful at: " 
                      << region.address << " (size: " << region.length << ")\n";
            ...
            if (sync_result) {
                ...
                std::cout << "File sync failed: " 
                          << error_to_string(sync_result.error().code) << "\n";
            }
            ...
            if (prefetch_result) {
                ...
                std::cout << "Memory prefetch failed: "
                          << error_to_string(prefetch_result.error().code) << "\n";
            }
            ...
            std::cout << "File-backed mapping failed: " 
                      << error_to_string(result.error().code) << "\n";
        }
    ...
    if (caps.supports_memory_lock) {
        ...
        if (result) {
            const auto& region = *result;
            std::cout << "Memory-locked mapping created, locked: " 
                      << (region.is_locked ? "yes" : "no") << "\n";
            ...
            if (lock_result) {
                ...
                if (unlock_result) {
                    ...
                    std::cout << "Memory unlocking failed: "
                              << error_to_string(unlock_result.error().code) << "\n";
                }
            } else {
                std::cout << "Manual memory locking failed: "
                          << error_to_string(lock_result.error().code) << "\n";
            }
    ...
    if (caps.supports_large_pages) {
        ...
        if (result) {
            ...
            switch (region.actual_pages) {
                case page_preference::system_default:
                    std::cout << "system default\n";
                    break;
                case page_preference::prefer_large:
                    std::cout << "large pages (preferred)\n";
                    break;
                case page_preference::require_large:
                    std::cout << "large pages (required)\n";
                    break;
            }
            
            unmap_memory(region);
        } else {
            std::cout << "Large page mapping failed: "
                      << error_to_string(result.error().code) << "\n";
        }
    }
